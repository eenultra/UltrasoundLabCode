    ...
    B. Set the path by doing one of the following:
        ...
        * Add the MCR directory to the environment variable by issuing 
        ...
        NOTE: When deploying standalone applications, it is possible 
              to run the shell script file run_LUARP.sh 
...
3. Collect the following files, based on what you are deploying: 
    ...
    NOTE: LUARP.ctf will be present only if you have chosen 
    ...
    All Platforms:
        * LUARP.ctf (if present)(Component Technology File archive,
          platform-dependent on target machine)

    Windows:
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          installer)
        * LUARP.exe (application created by MATLAB Compiler)

    UNIX:
        * MCRInstaller.bin (MATLAB Compiler Runtime installer)
        * LUARP (application created by MATLAB Compiler)
        * run_LUARP.sh (shell script that can be run to 
    ...
    All Platforms:
        ...
        * LUARP.dll (shared library, file extension varies by 
          platform)
        * LUARP.h (library header file)
    ...
    Windows:
        * LUARP.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)    
        * LUARP.xml (documentation files)
        * LUARP.pdb (program database file - if DEBUG option 
          selected)
        * LUARP.dll (component assembly file)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
    ...
    All Platforms: 
        * LUARP.jar (Java class files and compiled M-code)
    ...
    Windows:
        * LUARP.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)
        * LUARP_1_0.dll (COM component)
        * LUARP.xla (optionally created and placed in
          U:\PhD\Pete\My UARP\LUARP_1_1\LUARP\src directory by the user)  
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
